...
class predictMovies_forUser:
    ...
    def predictForUser(self, predicted_Ratings):
        ...
        final_Data = pd.DataFrame(final_Data,index=index)
        finals = final_Data.sort_values(3, ascending=False)
        finals.columns = ["MovieId", "Movie Name", "Genre", "Predicted Rating for User " + str(userId_toSearch)]
        print(finals)

        self.create_HTML_CSV(finals, "Recommendation_User")
